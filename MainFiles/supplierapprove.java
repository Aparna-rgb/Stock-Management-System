...
double sum = 0.0; // Use double instead of int
for (int i = 0; i < table2.getRowCount(); i++) {
    try {
        sum += Double.parseDouble(table2.getValueAt(i, 4).toString().trim()); // Convert safely
    } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(this, "Invalid number format in table. Please check the data.", "Error", JOptionPane.ERROR_MESSAGE);
        return; // Stop execution if an error occurs
    }
}
jLabel2.setText(String.format("%.2f", sum)); // Display formatted sum (e.g., "4450.00")
...
this.setVisible(false);
    ...
    try {
        ...
        if (selectedRowIndex == -1) { // Check if a row is selected
            JOptionPane.showMessageDialog(this, "Please select an order to approve!", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        String itemId = model.getValueAt(selectedRowIndex, 0).toString();
        int newQuantity = Integer.parseInt(model.getValueAt(selectedRowIndex, 2).toString());

        // Fetch current quantity
        String query = "SELECT quantity FROM inventory WHERE item_id = '" + itemId + "'";
        ...
        if (rs1.next()) {
            ...
            JOptionPane.showMessageDialog(this, "Order approved successfully! Inventory updated.", "Success", JOptionPane.INFORMATION_MESSAGE);
        } else {
...
//             
//        Class.forName("com.mysql.cj.jdbc.Driver");
//       c=DriverManager.getConnection("jdbc:mysql://localhost:3306/ghana","root","Ghana@081");
//       s=c.createStatement();
//
//  DefaultTableModel model = (DefaultTableModel) table2.getModel();   
//           int selectedRowIndex =table2.getSelectedRow();
